...
import StyledAbcTextCustomerGroupDateWrapper from './styles';
...
function AbcTextCustomerGroupDate (props: AbcTextCustomerGroupDateProps)  {
  const {
    getPConnect,
    value,
    validatemessage,
 ...
 let { readOnly = false, required = false, disabled = false } = props;
  ...
  if (displayMode === 'LABELS_LEFT' || displayMode === 'STACKED_LARGE_VAL' || displayMode === 'DISPLAY_ONLY') {
    ...
    switch(displayMode){
      case 'DISPLAY_ONLY': {
        return ( <StyledAbcTextCustomerGroupDateWrapper> {displayComp} </StyledAbcTextCustomerGroupDateWrapper>);
      }
      case "LABELS_LEFT" : {
        return (
          <StyledAbcTextCustomerGroupDateWrapper>
          <FieldValueList
...
export default withConfiguration(AbcTextCustomerGroupDate);
