...
import StyledAbcTextCustomerGroupParagraphWrapper from './styles';
...
function AbcTextCustomerGroupParagraph(props: AbcTextCustomerGroupParagraphProps) {
  const {
    getPConnect,
    value,
    hideLabel = false,
 ...
 // BUG-547602: Temporary type coercion for 8.5 until DXAPIs are enhanced to pass original pxViewMetadata JSON, respecting boolean primitives
  ...
  if (displayMode === 'LABELS_LEFT' || displayMode === 'DISPLAY_ONLY') {
    ...
    return displayMode === 'DISPLAY_ONLY' ? (
      <StyledAbcTextCustomerGroupParagraphWrapper>
      {displayComp}
      </StyledAbcTextCustomerGroupParagraphWrapper>
    ) : (
      <StyledAbcTextCustomerGroupParagraphWrapper>
...
export default withConfiguration(AbcTextCustomerGroupParagraph);
